...
public class courseRepository extends baseRepository<Course> {
    ...
    public courseRepository(File f) {
        super(f);
    }
    ...
    protected Course fromCSV(String[] cols) {
        ...
        if (cols.length > 3) {
            try {
                c.setCredit(Integer.parseInt(cols[3].trim()));
    ...
    protected String toCSV(Course c) {
        String courseCode = safe(c.getCode());
        String name   = safe(c.getName());
        String credit = String.valueOf(c.getCredit());
        String wq = NumberFormatter.formatCSV(c.getwQuiz());
        String wm = NumberFormatter.formatCSV(c.getwMid());
        String wf = NumberFormatter.formatCSV(c.getwFinal());
        String wp = NumberFormatter.formatCSV(c.getwProject());
        int rowId = items.indexOf(c) + 1;

        return String.join(",", String.valueOf(rowId), courseCode, name, credit, wq, wm, wf, wp);
    }
    ...
    protected String[] getHeader() {
        return new String[] {
                ...
                "weightQuiz","weightMid","weightFinal","weightProject"
        };
    }
